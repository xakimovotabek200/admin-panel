            ...
            <form className="nimadur" onSubmit={handleSubmit((formData, e) => {
              ...
              <div className="items-center md:flex">
                <input
                  ...
                  className="w-[320px] ml-1 md:w-52 h-10 px-4 py-2 border-2 border-[#dee2e6] rounded-lg focus:outline-none focus:border-blue-400 placeholder-gray-600 md:ml-5"
                  {...register("text")}
                  value={inputValue} // Set the value of the input field
                  onChange={(e) => setInputValue(e.target.value)} // Update input value
                />
                <button type="submit" className="w-[320px] ml-1 mt-3 md:w-52 md:mt-0 h-10 px-4 py-[5px] flex border-2 border-blue-400 text-blue-400 rounded-lg md:ml-5 hover:bg-blue-400 md:items-center md:justify-center hover:border-blue-400 hover:text-white">Ma'lumot qo'shish</button>
             </div>
            </form>
          </div>
        </div>
        <div>
          <div className=" ml-1 md:w-[50rem] grid gap-4 md:ml-5 max-h-[670px] overflow-y-auto mt-5">
            {data.length > 0 &&
              data.map((item) => (
                <div key={item.id} className="w-[320px] md:flex md:w-full items-center justify-evenly border-2 border-[#dee2e6] p-2">
                  <div className="md:w-full md:flex md:items-center md:justify-between">
                    <p className=" text-gray-700 font-bold">{item.text}</p>
                    <div className="mt-2 md:mt-1 flex gap-11 md:flex md:gap-11">
                      <button
                  ...
                  {editingId !== null && editingId === item.id ? (
                    <form
                      ...
                      className=""
                    >
                      <div className="mt-2 flex gap-5 md:flex md:gap-11">
                        <input
                          type="text"
                          defaultValue={item.text}
                          {...register("text")}
                          className="w-32 md:flex h-10 md:w-52 items-center justify-evenly rounded-lg border-2 border-[#dee2e6] px-4 py-2 text-gray-600 placeholder-gray-600 focus:outline-none"
                        />
                        <button type="submit" className="w-32 flex items-center justify-evenly md:flex h-10 md:w-44 rounded-lg border-2 border-gray-400 px-4 py-2 text-black"><GrDocumentUpdate /> Update</button>
                      </div>
                    </form>
                  ) : (
                      <p></p>
