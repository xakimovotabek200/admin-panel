          ...
          <div className="flex flex-wrap gap-4 md:block">
            ...
            <div className="items-center md:flex">
              <input
                ...
                className="w-[320px] ml-1 md:w-52 h-10 px-2 py-[3px] text-gray-600 border-2 border-[#dee2e6] bg-white rounded-lg focus:outline-none focus:border-blue-400 md:ml-5"
                {...register("amount")}
              />
              <select name="" id="" {...register("product_slug")} className="w-[320px] ml-1 mt-3 md:w-52 h-10 px-4 py-2 border-2 border-[#dee2e6] rounded-lg focus:outline-none focus:border-blue-400 text-gray-600 md:mt-0 md:ml-2">
                {Array.isArray(datas) ? (
                  datas.map((item) => (
              ...
              <input
                ...
                {...register("end_time")}
              />
                <button type="submit" className="w-[320px] ml-1 mt-3 md:ml-2 md:col-span-6 md:w-52 md:mt-0 h-10 border border-blue-400 bg-blue-500 text-white text-lg rounded-lg"><span>Submit Data</span></button>
            </div>
          </form>

          <h2 className="text-2xl mb-2 ml-5 md:mt-3 font-bold text-gray-600">Existing Discounts</h2>
            <div className="ml-1 md:ml-5 max-h-[670px] overflow-y-auto">
            {discounts.map((discount) => (
                <>
                <div key={discount.id} className="mb-5 border border-black p-3 flex md:items-center flex-wrap justify-between">
                  ...
                  <p className="ml-10 text-[18px] md:text-xl font-bold text-gray-600 mt-2">from {discount.start_time}</p>
                  <p className="ml-10 text-[18px] md:text-xl font-bold text-gray-600 mt-2">to {discount.end_time}</p>
                  <div className="mt-2 flex gap-3 md:flex md:items-center md:justify-evenly">
                ...
                {editingId !== null && editingId === discount.id && (
                  <form className="discount_info" onSubmit={handleSubmit(onEdit)}>
                    <div className="items-center md:flex">
                      <input
                        type="number"
                        placeholder="Discount Amount"
                        className="w-[320px] ml-1 md:w-52 h-10 px-2 py-[3px] text-gray-600 border-2 border-[#dee2e6] bg-white rounded-lg focus:outline-none focus:border-blue-400 md:ml-0"
                        {...register("amount")}
                      />
                      <select name="" id="" {...register("product_slug")} className="w-[320px] ml-1 mt-3 md:w-52 h-10 px-4 py-2 border-2 border-[#dee2e6] rounded-lg focus:outline-none focus:border-blue-400 text-gray-600 md:mt-0 md:ml-2">
                        {Array.isArray(datas) ? (
                      ...
                      <input
                        ...
                        {...register("end_time")}
                      />
                      <button type="submit" className="w-[320px] ml-1 mt-3 md:ml-2 md:col-span-6 md:w-52 md:mt-0 h-10 border border-blue-400 bg-blue-500 text-white text-lg rounded-lg"><span>Update Data</span></button>
                    </div>
                  </form>
                )}
              </>
