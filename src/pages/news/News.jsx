          ...
          <div className="flex flex-wrap gap-4 md:block">
            ...
            >
              <div className="items-center md:flex">
                <input
                  type="file"
                  className="w-[320px] ml-1 md:w-52 h-10 px-2 py-[3px] text-gray-600 border-2 border-[#dee2e6] bg-white rounded-lg focus:outline-none focus:border-blue-400 md:mt-0 md:ml-5"
                  {...register("image")}
                />
                <input
                  type="text"
                  placeholder="Text"
                  className="w-[320px] ml-1 mt-3 md:w-52 h-10 px-4 py-2 border-2 border-[#dee2e6] rounded-lg focus:outline-none focus:border-blue-400 placeholder-gray-600 md:mt-0 md:ml-2"
                  {...register("text")}
                  defaultValue={""}
                />
                <input
                ...
                <input
                  ...
                  defaultValue={"time"}
                />
                <button className="w-[320px] ml-1 mt-3 md:col-span-6 md:mt-0 md:ml-3 md:w-52 h-10 border border-blue-400 bg-blue-500 text-white text-lg rounded-lg">
                  <span>Ma'lumotlarni yuborish</span>
                </button>
              </div>
            </form>
            <div className="ml-1 md:min-h-96 overflow-y-auto md:mt-5 md:ml-5">
              {Array.isArray(data) ? (
                data.map((item) => (
                  <div>
                    <div key={item.id} className="w-[320px] md:flex md:w-[60rem] items-center justify-between border-2 border-[#dee2e6] p-1">
                      <img className=" w-[320px] object-cover md:w-20 h-32" src={`https://api.abdullajonov.uz/legend-backend-api/public/storage/images/${item.image}`} alt="" />
                      <div className="flex mt-2 ml-[-10px] md:flex items-center justify-center gap-5">
                        <p className=" text-[15px] text-gray-600 md:text-[20px]">{item.text}</p>
                        <p className="text-[15px] text-gray-600 md:text-[20px]">{item.title}</p>
                      </div>
                      <div className="mt-2 flex gap-5 md:flex">
                    ...
                    <div className="flex flex-wrap gap-4 md:flex mt-3">
                      {editingId !== null && editingId === item.id && (
                        ...
                        >
                          <div className="items-center md:flex">
                            <input
                              type="file"
                              className="w-[320px] ml-1 md:w-52 h-10 px-2 py-[3px] text-gray-600 border-2 border-[#dee2e6] bg-white rounded-lg focus:outline-none focus:border-blue-400 md:mt-0 md:ml-0"
                              {...register("image")}
                            />
                            <input
                              type="text"
                            ...
                            />
                            <button className="w-[320px] ml-1 mt-3 md:col-span-6 md:w-52 md:mt-0 md:ml-2 h-10 border border-blue-400 bg-blue-500 text-white text-lg rounded-lg">
                              <span>Ma'lumotlarni yuborish</span>
                            </button>
                          </div>
                        </form>
